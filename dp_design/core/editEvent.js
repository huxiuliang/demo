function editEvent(eventValue, callback) {
    ...
    $("#saveEventBtn").bind("click", function () {
        console.log("saveEventBtn click");
        if (callback) {
            callback.call(this, eventEditor.getValue());
        }
        app.jsedit.status = false;
        $("#saveEventBtn").unbind("click");
    });

    $.getJSON("dp_design/common-event/common.json", {},
        function (data, textStatus, jqXHR) {
            console.log(data)
            var commonEvents = $("#commonEvents");
            commonEvents.empty();
            $.each(data.events, function (i, val) {
                var li = $('<li title="' + val.desc + '">' + val.name + '</li>');
                li.data(val);
